...
def make_alert(s):
    a={"source":s["name"],"priority":"HIGH","category":"Official updates","title":f"Check latest {s['name']} notifications","summary":"Open the official source for the latest admission and counselling updates.","date":datetime.now().strftime("%d %b %Y"),"url":s["url"]}
    a["id"]=hashlib.md5(f"{a['source']}|{a['title']}|{a['date']}".encode()).hexdigest()[:12]
    return a
ALERTS=[make_alert(s) for s in SOURCES]
@app.get("/")
